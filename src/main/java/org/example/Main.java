...
public class Main {
    public static void main(String[] args) throws IllegalAccessException {
        ...
        for (var entry : JoorReflectUtils.toMap(a).entrySet()) {
            System.out.printf("%s %s\n", entry.getKey(), entry.getValue());
        }
        var b = new B();
        JoorReflectUtils.fromMapSetFields(JoorReflectUtils.toMap(a), b);
        System.out.println(b);

        var c = new C();
        JoorReflectUtils.convert(a, c, Map.of("a", "A", "b", "B", "c", "C"),
                Map.of("c", String::valueOf));
        System.out.println(c);

        b = new B();
        var converter = JoorReflectUtils.getConverter(a, B.class, Map.of());
